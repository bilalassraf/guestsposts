...
class UserRequest extends Model
{
    ...
    public function getlessCoodinatorAttribute()
    {
        ...
        $min_val = UserRequest::where('web_name', $this->web_name)->with('categories')->min('price');
        $max_val = UserRequest::where('web_name', $this->web_name)->with('categories')->max('price');
        $min_record = UserRequest::where('web_name', $this->web_name)->where('price', '=', $min_val ?? '')->with('categories')->first();
        $max_record = UserRequest::where('web_name', $this->web_name)->where('price', '=', $max_val ?? '')->with('categories')->first();
    ...
    {
        $min_val = UserRequest::where('web_name', $domain)->with('categories')->min('price');
        $max_val = UserRequest::where('web_name', $domain)->with('categories')->max('price');
        $min_record = UserRequest::where('web_name', $domain)->where('price', '=', $min_val ?? '')->with('categories')->first();
        $max_record = UserRequest::where('web_name', $domain)->where('price', '=', $max_val ?? '')->with('categories')->first();
